...
extension ExploreViewModel {
    ...
    public func pullSortedPublicSets() {
        db.collection("userSets").whereField("isPublic", isEqualTo: true).order(by: filterBy, descending: isDescending).limit(to: self.allPublicSets.count).getDocuments { (snap, error) in
            if error != nil {
    ...
    public func pullSortedPrivateSets() {
        db.collection("userSets").whereField("isPublic", isEqualTo: false).order(by: filterBy, descending: isDescending).limit(to: self.allPrivateSets.count).getDocuments { (snap, error) in
            if error != nil {
                print(error!.localizedDescription)
            } else {
                guard let data = snap?.documents else { return }
