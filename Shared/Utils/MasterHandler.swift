...
class MasterHandler: ObservableObject {
    ...
    func setAlertSettings(alertAction: @escaping () -> () = { return }, alertTitle: String = "", alertSubtitle: String = "", hasCancel: Bool = true, actionLabel: String = "") {
...
struct CountryCodes {
    ...
    var countryCodeDict: [String:[String]] {
        return ["AF": ["Afghanistan","93"],
    ...
    var getCCList: [CountryCode] {
        var codesList = [CountryCode]()
        for pair in self.countryCodeDict {
            codesList.append(CountryCode(countryFullName: pair.value[0], countryAbbreviation: pair.key, code: pair.value[1]))
        }
        return codesList
    }
}
...
struct CountryCode: Hashable {
    ...
    init(countryFullName: String, countryAbbreviation: String, code: String) {
