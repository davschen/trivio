//
//  GameFlashcardsCategoriesView.swift
//  Trivio! (iOS)
//
//  Created by David Chen on 1/22/23.
//

import Foundation
import SwiftUI

struct GameFlashcardsCategoriesView: View {
    ...
    var body: some View {
        VStack {
            ...
            TabView(selection: $currCategoryIndex) {
                ForEach(gamesVM.flashcardClues2D.indices, id: \.self) { categoryIndex in
                    GameFlashcardsSingleCategoryView(
        ...
        .fullScreenCover(isPresented: $isPresentingFlashcardsView) {
            FlashcardsView(currCategoryIndex: $currCategoryIndex, topCardClueIndex: $topCardClueIndex, bottomCardClueIndex: $bottomCardClueIndex, cardIndicesToStudy: $cardIndicesToStudy, studyingClueSide: $studyingClueSide)
        }
    }
...
struct GameFlashcardsSingleCategoryView: View {
    @EnvironmentObject var formatter: MasterHandler
    @EnvironmentObject var gamesVM: GamesViewModel
...
struct FlashcardsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            SingleFlashcardView(xOffset: $xOffset, isShowingResponse: $isShowingResponse, studyingClueSide: $studyingClueSide, flashcardClue: bottomClue, isTopCard: false)
                .opacity((cardIndicesToStudy.count <= 1 && !xOffsetIsAnimating) ? 0 : 1)
            SingleFlashcardView(xOffset: $xOffset, isShowingResponse: $isShowingResponse, studyingClueSide: $studyingClueSide, flashcardClue: topClue, isTopCard: true)
                .opacity(xOffsetIsAnimating ? 0 : 1)
                .overlay(RoundedRectangle(cornerRadius: 20).stroke(xOffsetIsAnimating ? .clear : bgColor, lineWidth: 2).padding(.bottom, 25))
